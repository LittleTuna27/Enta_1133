...
using static Dugneon_Movers.Scripts.RoomBase;
...
namespace Dugneon_Movers.Scripts
{
    public class base_room_script
    {
        ...
        { new Room1(), new Room2(), new Room3(), new Room4(), new Room5(), new Room6(), new Room7(), new Room8(), new Room9()};
            ...
            grid[0, 0] = randomizedRooms[0]; // Top left
            grid[0, 1] = randomizedRooms[1]; // Top middle
            grid[0, 2] = randomizedRooms[2]; // Top right

            grid[1, 0] = randomizedRooms[3]; // Middle left
            grid[1, 1] = randomizedRooms[4]; // Middle middle (Start Room)
            grid[1, 2] = randomizedRooms[5]; // Middle right

            grid[2, 0] = randomizedRooms[6]; // Bottom left
            grid[2, 1] = randomizedRooms[7]; // Bottom middle
            grid[2, 2] = randomizedRooms[8]; // Bottom right
        }
            ...
            if (int.TryParse(input, out int choice))
            {
                switch (choice)
                {
                    case 1:
                        ...
                        if (player != null)
                        {
                            player.DisplayInventory();  // Use the player object passed from GameManager
                        }
                        else
                        {
